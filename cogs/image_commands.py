...
class ImageCommands(commands.Cog):
    ...
    @commands.slash_command(contexts={discord.InteractionContextType.private_channel, discord.InteractionContextType.guild}, integration_types={discord.IntegrationType.user_install, discord.IntegrationType.guild_install}, name="sign", description="generate mario 64 sign image")
    async def sign(self, ctx, user: discord.Member = None):
        if user == None:
            user = ctx.author
        sign = Image.open("images/1707.png")
...
#
    ...
    @commands.slash_command(contexts={discord.InteractionContextType.private_channel, discord.InteractionContextType.guild}, integration_types={discord.IntegrationType.user_install, discord.IntegrationType.guild_install}, name="cool_image",description="randomly generate an image")
    async def cool_image(self, ctx, image = None):
        if image == None:
            selected_file = random.choice(os.listdir("images/"))
            img = Image.open("images/" + selected_file)
    ...
    @commands.slash_command(contexts={discord.InteractionContextType.private_channel, discord.InteractionContextType.guild}, integration_types={discord.IntegrationType.user_install, discord.IntegrationType.guild_install}, name="petpet", description="pet a user's avatar or an emoji")
    async def petpet(self, ctx, image: Optional[Union[discord.PartialEmoji, discord.User]]):
        if type(image) == discord.PartialEmoji:
            image = await image.read() # retrieve the image bytes
        elif type(image) == discord.User:
